...
def delete_process(process_id):
    try:
        ...
        # If the user is an Industry Manager, restrict the deletion to their own processes
        if g.role == 'Industry Manager':
            cursor.execute("""
                SELECT * 
                FROM process 
                WHERE process_id = %s 
                  AND industry_id = (SELECT industry_id FROM user WHERE username = %s)
            """, (process_id, g.username))
            process = cursor.fetchone()
            if not process:
                flash("You do not have permission to delete this process.", "danger")
                return redirect(url_for('view_processes'))
